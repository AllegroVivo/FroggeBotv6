...
FP = TypeVar("FP", bound="FormPrompt")

################################################################################
class FormPrompt(Identifiable):
...
################################################################################
    def __init__(self, parent: Form, _id: int, _type: DisplayTime, **kwargs) -> None:

        super().__init__(_id)
        self._parent: Form = parent

        self._prompt_type: DisplayTime = _type
...
################################################################################
    @classmethod
    def new(cls: Type[FP], parent: Form, _type: DisplayTime) -> FP:

        new_id = parent.bot.api.create_form_prompt(parent.id, _type.value)
        return cls(parent, new_id, _type)

################################################################################
    @classmethod
    def load(cls: Type[FP], parent: Form, _type: DisplayTime, data: Dict[str, Any]) -> FP:
...
################################################################################
    def update(self) -> None:
        ...
        self.bot().api.update_form_prompt(self)
